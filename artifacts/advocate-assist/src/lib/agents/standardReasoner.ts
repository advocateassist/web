// ── Standard Reasoner — DeepSeek V4 Pro (deepseek-reasoner) ──────────────────
//
// Handles complexReasoning queries classified as STANDARD complexity:
//   trademark filings, IP rights, consumer disputes, compliance FAQs,
//   general rights queries, district court matters, property, employment, tax.
//
// Fallback chain: deepseek-reasoner → deepseek-chat
//
// Reserve eliteReasoner (Claude Sonnet 4.6) for ELITE complexity only —
// appellate SC/HC research, multi-jurisdictional CoL, complex litigation strategy.
...
export async function standardReasonerNode(state: AgentState): Promise<Partial<AgentState>> {
...
${eraInstruction
    ? `ERA RULE (ENFORCED): ${eraInstruction}`
    : "ERA STATUS: Incident date not yet determined — do NOT ask again if the intake checklist was already sent. Wait for user's reply."}

${isDraftRequest
    ? `DRAFTING RULE: Generate the COMPLETE document from start to finish.
Use all available case facts — do NOT use placeholder text such as "[Name]" or "[Address]".
Produce a well-structured, court-ready draft in its entirety.`
    : ""}
